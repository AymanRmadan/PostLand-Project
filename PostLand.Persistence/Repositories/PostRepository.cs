...
namespace PostLand.Persistence
{
    public class PostRepository : BaseRepository<Post>, IPostRepository
    {
        ...
        public async Task<IReadOnlyList<Post>> GetAllPostsAsync(bool includeCategory)
        {
            ...
            allPosts = includeCategory == true ? await _dbContext.Posts.Include(x => x.Category).ToListAsync() : await _dbContext.Posts.ToListAsync();
            return allPosts;
        }

        public async Task<Post> GetPostByIdAsync(int id, bool includeCategory)
        {
            Post Post = new Post();
